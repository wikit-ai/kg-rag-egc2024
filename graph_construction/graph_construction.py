...
transformations_dataset = 'graph_construction/transformations.xlsx'
...
graph.run("""CALL apoc.load.json("https://unstats.un.org/SDGAPI/v1/sdg/Goal/List?includechildren=false")
    YIELD value
...
for index, row in df_int.iterrows():
    params_dict={'code':int(row['Transformation']),'int_title':str(row['Interventions']),
...
for index, row in df.iterrows():
    params_dict={'code':str(row['SDG']), 'weight':int(row['Weight']), 'desc':transf_dict[str(row['Weight'])], 'io_title':str(row['Outputs'])}
    params.append(params_dict)
...
for index, row in df_min.iterrows():
    params_dict={'code':int(row['Transformation']),
